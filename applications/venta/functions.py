...
def procesar_venta(self, **params_venta):
    ...
    if productos_en_car.count() > 0:
        ...
        # ⚠️ VALIDACIÓN SIMPLE DE STOCK
        for producto_car in productos_en_car:
            if producto_car.count > producto_car.product.count:
                # Podrías lanzar una excepción o simplemente retornar None
                raise ValueError(
                    f"Stock insuficiente para el producto '{producto_car.product.name}'. "
                    f"Disponible: {producto_car.product.count}, solicitado: {producto_car.count}"
                )
